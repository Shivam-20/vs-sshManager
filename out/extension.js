...
const configParser_1 = require("./services/configParser");
const knownHostsManager_1 = require("./services/knownHostsManager");
const sshAgentManager_1 = require("./services/sshAgentManager");
const treeDataProvider_1 = require("./ui/treeDataProvider");
function activate(context) {
    const sshPath = vscode.workspace.getConfiguration('sshManager').get('sshPath') || '~/.ssh';
    const keyManager = new keyManager_1.KeyManager(sshPath);
    const configParser = new configParser_1.ConfigParser(sshPath);
    const knownHostsManager = new knownHostsManager_1.KnownHostsManager(sshPath);
    const agentManager = new sshAgentManager_1.SSHAgentManager();
    const treeDataProvider = new treeDataProvider_1.SSHManagerTreeDataProvider(keyManager, configParser, knownHostsManager, agentManager);
    ...
    context.subscriptions.push(refreshCmd, generateKeyCmd, addHostCmd, copyPublicKeyCmd, deleteKeyCmd, editHostCmd, deleteHostCmd, removeKnownHostCmd, copySshCommandCmd, testConnectionCmd, openTerminalCmd, viewPublicKeyCmd, openSshFolderCmd, openConfigFileCmd, changePassphraseCmd, removePassphraseCmd);
}
function deactivate() { }
